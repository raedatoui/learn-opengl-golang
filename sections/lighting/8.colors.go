...
func (lc *LightingColors) InitGL() error {
	...
	if sh, err := utils.Shader("_assets/8.lighting_colors/colors.vs", "_assets/8.lighting_colors/colors.frag", ""); err != nil {
		return err
	} else {
		lc.lightingShader = sh
	}
	if sh, err := utils.Shader("_assets/8.lighting_colors/lamp.vs", "_assets/8.lighting_colors/lamp.frag", ""); err != nil {
		return err
	} else {
		lc.lampShader = sh
	}
